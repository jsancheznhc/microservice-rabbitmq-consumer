 ...
 */
@Service
public class ListenerHystrixSync implements RabbitListenerConfigurer {
	...
	public void listener(final Message message, Channel channel)
			throws GenericListenerException, IOException, Exception {
		// clear MDC for every message and fill again
		MDC.clear();
		int retry = getRetryCount(message.getMessageProperties().getHeaders());
		...
		LoggerManager
				.getLogger(ListenerHystrixSync.class, "LBL_RECIEVE_MESSAGE_CONSUME_TIME", Status.DIAGNOSTIC.getCode())
				.debug("{}, Message Received at {}: ", Status.DIAGNOSTIC.getDescription(), messageTime());

		LoggerManager.getLogger(ListenerHystrixSync.class, "LBL_RECIEVE_MESSAGE", Status.DIAGNOSTIC.getCode()).debug(
				"{}, messagebody retrieved :{}", Status.DIAGNOSTIC.getDescription(),
				new String(message.getBody(), StandardCharsets.UTF_8));
		// control retries

		if (validateHeaders(headerString)) {
			// Reached the max retry attempt
			if (continueRetrying(retry)) {
	...
	public void listenerFallback(final Message message, Channel channel)
			throws GenericListenerException, IOException, Exception {
		LoggerManager
	...
	private Map<String, String> buildHeaders(final Message message) {
		...
		if (headersObject.containsKey(X_DEATH)) {
			int retry = getRetryCount(headersObject);
			LoggerManager
					.getLogger(ListenerHystrixSync.class, "LBL_RECIEVE_MESSAGE_COUNT_VALUE",
							Status.DIAGNOSTIC.getCode())
	 ...
	 * @return
	 */
	private boolean continueRetrying(int retry) {
		if (retry > Integer.parseInt(maxRetry)) {
			LoggerManager
					.getLogger(ListenerHystrixSync.class, "LBL_RECIEVE_MESSAGE_RETRY_COUNT",
							Status.FISNISHED_RETRIES.getCode())
					.info("{}, Retry Count value [{}] greater than max retry [{}] configured, send DLQ status",
							Status.FISNISHED_RETRIES.getDescription(), retry, maxRetry);
			return false;
		}
		return true;
	}
	...
	private Status callRestClient(RestRequest request, final Message message, Channel channel)
			throws GenericListenerException, IOException, Exception {
		statusValue = restClientService.consume(request);
	...
	private Status callRestClientFallback(RestRequest request, final Message message, Channel channel)
			throws GenericListenerException, IOException, Exception {
		LoggerManager.getLogger(ListenerMessageReceiver.class, "LBL_FALLBACK_CIRCUIT_BREAKER_OPEN")
				.info("CAll [{}], channel [{}]", callNum, channel.getChannelNumber());
		acklogic.ackLogic(message, Status.RETRY, channel);
		return Status.RETRY;
	}
	...
	private void fillMDC(Map<String, String> headers, long retry) {
		MDC.clear();
		LoggerManager.getLogger(ListenerHystrixSync.class, "LBL_HEADERS", Status.DIAGNOSTIC.getCode())
		...
		LoggerManager.getLogger(ListenerHystrixSync.class, "LBL_HEADERS_MDC", Status.DIAGNOSTIC.getCode())
		...
		if (xDeath != null) {
			for (HashMap<String, Object> items : xDeath) {
				if (MESSAGE_STATUS_REJECTED.equals(items.get(MESSAGE_REASON))) {
					count = ((Long) items.get("count")).intValue();
					count++;
				}
			}
		}
